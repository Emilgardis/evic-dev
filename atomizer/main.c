...
#define LEFT 2
#define FPS 15
volatile uint32_t buttonSpec[3][3] = {
    {0}, {0}, {0}}; // [timesPressed, justPressed, timerWhenUpdate]
volatile uint32_t
    timer = 0,
    timer2; // TODO: Handle overflow. Will Currently last 4.97 days.
volatile uint32_t newWatts = 0; // Is this safe?
volatile uint8_t newWatts_Open =
    1; // If this is true, then newWatts can be modified.

void timerCallback() { timer++; }

void timer2Callback() { timer2++; }
...
uint16_t correctVoltage(uint16_t currentVolt, uint32_t currentWatt,
                        uint16_t res) {
...
void buttonFireCallback(
    uint8_t state) { // Only gets called when something happens.
...
int main() {
  ...
  //   Add a small snake game. (?) What would the memory impact be? Trigger?
  //   left left right right fire 2x.
  //   See how puff and time is implemented and use these mem locations for
  //   something cool.
  //   Add volt mode. (?)
  char buf[200];
  uint8_t mode = 0;
  ...
  // Show logo!
  Display_PutPixels(0, 32, Bitmap_evicSdk, Bitmap_evicSdk_width,
                    Bitmap_evicSdk_height);
  Display_Update();
  Timer_DelayMs(500);
  // Main loop!
  newWatts = watts;
  while (1) {
    ...
    if ((btnState >= BUTTON_MASK_LEFT + BUTTON_MASK_RIGHT) ||
        (btnState == BUTTON_MASK_LEFT + BUTTON_MASK_FIRE) ||
        (btnState == BUTTON_MASK_RIGHT + BUTTON_MASK_FIRE)) {
      newWatts_Open = 0;
    } else if (mode == 0) {
      newWatts_Open = 1;
    } else {
    ...
    if (!sleeping && buttonSpec[FIRE][0] >= 3 && buttonSpec[FIRE][1] == 1 &&
        timer - buttonSpec[FIRE][2] < 20) { // can we try zero here?
      if (buttonSpec[FIRE][0] == 3) {
        ...
        // Now we go to sleep. Sleep lasts for 2 minutes, then it goes into
        // power off.
        if (!sleeping) {
          Display_SetOn(0);
          sleeping = 1;
    ...
    if (!Atomizer_IsOn() && (btnState == BUTTON_MASK_FIRE) &&
        buttonSpec[FIRE][1] && // Only fire if fire is pressed alone.
    ...
    if (mode == 0) {
      if (timer2 - lastTime > FPS) {
        siprintf(buf, "P:%3lu.%luW\nV:%3d.%02d\n%1d.%02d "
                      "Ohm\nBV:%uV\nI:%2d.%02dA\n%s\n%s\n%d %d %d\n%d\n",
                 watts / 1000, watts % 1000 / 100, displayVolts / 1000,
                 displayVolts % 1000 / 10, atomInfo.resistance / 1000,
                 atomInfo.resistance % 1000 / 10, Battery_GetVoltage(),
                 atomInfo.current / 1000, atomInfo.current % 1000 / 10,
                 atomState, batteryState, buttonSpec[LEFT][0],
                 buttonSpec[FIRE][0], buttonSpec[RIGHT][0], mode);
        Display_Clear();
        Display_PutText(0, 13, buf, FONT_DEJAVU_8PT);
        Display_PutPixels(0, 0, mode0_bitmap, mode0_bitmap_width,
                          mode0_bitmap_height);
        Display_Update();
        lastTime = timer2;
      }
    ...
    if (mode == 1 && 0) { // FIXME: not implemented yet.
      // Here we can adjust all the settings, eg. change variables like
      // increment and maybe
      // TC values (when implemented). We should also see if we can store
      // different configs.
    }
    ...
    Timer_DelayMs(10); // Some delay, should keep us stable.
  }
}
